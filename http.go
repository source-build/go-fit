...
func (r *HttpUtil) NewRequest(methods RequestMethods, url string, body string, header ...H) *HttpUtil {
	request, err := http.NewRequest(string(methods), url, strings.NewReader(body))
	if len(header) > 0 {
